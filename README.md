...
| | CC Switch | CC-Gate |
|---|---|---|
| 切换方式 | **切换"供应商"**——一次只能激活 DeepSeek 或 GLM 或 Qwen，切模型必须先切供应商 | **所有厂商的模型平等地存在于一个列表里**——直接在对话里 `/model` 切，不需要切供应商 |
| 跨厂商切换 | ❌ 不能。选中 DeepSeek 就只能用 DeepSeek 的模型 | ✅ 任意切换。上一句话用 DeepSeek，下一句话切到 GLM，代理自动按模型名路由 |
| 模型列表 | 单厂商内部模型 | 全厂商模型混合，用户勾选哪些就显示哪些 |

**一句话：CC Switch 是"先选店再点菜"，CC-Gate 是"所有菜在一张菜单上"。** 代理层自动识别模型名、自动路由到对应厂商，用户不需要知道"供应商"这个概念。

```
CC Switch 的流程：
  用户 → 选供应商(DeepSeek) → 只能看到 DeepSeek 模型 → 选模型 → 用
        ↘ 想用 GLM？→ 退出 → 切换供应商(GLM) → 重新选模型 → 用

CC-Gate 的流程：
  用户 → 直接在对话里 /model → 看到所有厂商的模型混排 → 选任意一个 → 代理自动路由
  不需要"切换供应商"这个动作——因为根本没有供应商这个概念。
```
...
CC-Gate 的设计深受 [CC Switch](https://github.com/cexll/myclaude) 的启发。我们将"供应商切换"的理念升级为"所有模型平等存在于一个目录"，并扩展到了 10 个 Agent。
