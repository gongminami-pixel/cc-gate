...
| | CC Switch | CC-Gate |
|---|---|---|
| CLI 多窗口并行 | ❌ **全局切换、单模型独占**。切了供应商后，所有终端窗口都只能用这一个模型 | ✅ **每个 alias 绑定不同模型，多窗口同时并行**。`codex-ds` 用 DeepSeek、`codex-glm` 用 GLM、`codex-mimo` 用 MiMo——3 个终端窗口 3 个模型同时跑，互不干扰 |
| 切换机制 | 切换**供应商**——想换模型必须先切供应商，全局生效 | 切换 **alias**——想用哪个模型直接敲对应的 alias，换模型 = 换一个终端窗口 |
| 模型列表 | 单厂商内部模型 | 全厂商模型混排，用户勾选哪些就显示哪些 |

**CC Switch 是一把遥控器，换台所有人都得跟着看。CC-Gate 是每个人手里都有自己的遥控器。**
